...
async def reg_ip(event):
    ...
    if a == "true":
        ...
        await event.answer("Contact @darkanonn", alert=True)
...
async def point(event):
    async def point_(event):
        ...
        async with bot.conversation(chat) as pw1:
            ...
            pw1 = pw1.wait_event(events.NewMessage(
                incoming=True, from_users=sender.id))
            pw1 = (await pw1).raw_text
        async with bot.conversation(chat) as exp:
            await event.respond("**IP-VPS:**")
            exp = exp.wait_event(events.NewMessage(
                incoming=True, from_users=sender.id))
